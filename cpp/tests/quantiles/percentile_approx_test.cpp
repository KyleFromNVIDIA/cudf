  ...
  for (int idx = 0; idx < 2; idx++) {
    ...
    for (size_t v_idx = 0; v_idx < values.size(); v_idx++) {
      ...
      auto reduce = [stream](cudf::column_view const& values, int delta) {
        ...
        return cudf::make_structs_column(tbl.num_rows(), std::move(cols), 0, cudf::create_null_mask(0, cudf::mask_state::UNALLOCATED));
      };
      // groupby path
...
std::pair<cuda::device_buffer<uint8_t>, cudf::size_type> make_null_mask(cudf::column_view const& col)
{
