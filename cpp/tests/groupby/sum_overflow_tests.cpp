...
TYPED_TEST(groupby_sum_overflow_test, Basic)
{
  ...
  if constexpr (cudf::is_fixed_point<V>()) {
    ...
    auto expect_vals = cudf::create_structs_hierarchy(3, std::move(children), 0, cudf::create_null_mask(0, cudf::mask_state::UNALLOCATED));
...
TYPED_TEST(groupby_sum_overflow_test, EmptyCols)
{
  ...
  children.push_back(sum_col.release());
  children.push_back(overflow_col.release());
  auto expect_vals = cudf::create_structs_hierarchy(0, std::move(children), 0, cudf::create_null_mask(0, cudf::mask_state::UNALLOCATED));
...
TYPED_TEST(groupby_sum_overflow_test, MultiBlock)
{
  ...
  if constexpr (cudf::is_fixed_point<V>()) {
    ...
    children.push_back(sum_col.release());
    children.push_back(overflow_col.release());
    auto expect_vals = cudf::create_structs_hierarchy(num_groups, std::move(children), 0, cudf::create_null_mask(0, cudf::mask_state::UNALLOCATED));
    test_single_agg(keys,
                    vals,
                    expect_keys,
                    *expect_vals,
