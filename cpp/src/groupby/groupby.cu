...
struct empty_column_constructor {
  ...
  std::unique_ptr<cudf::column> operator()() const
  {
    ...
    if constexpr (k == aggregation::Kind::COLLECT_LIST || k == aggregation::Kind::COLLECT_SET ||
      ...
      return make_lists_column(0, make_empty_column(type_id::INT32), empty_like(values), 0, cudf::create_null_mask(0, cudf::mask_state::UNALLOCATED));
    }
    ...
    if constexpr (k == aggregation::Kind::SUM_OVERFLOW) {
      ...
      return make_structs_column(0, std::move(children), 0, cudf::create_null_mask(0, cudf::mask_state::UNALLOCATED), stream, mr);
    }
